...
题目只是评测样本，不携带题目专属 criterion。每条样本通过 `dimension_id` 选择统一维度库中的一套 Rubric。
...
准确性维度需要 `reference_context` 或 `standard_answer` 支持核验；两者均不足时应判“不适用”。
...
`score_result.py` 校验字段和取值，并追加：

- `rubric_version`：维度库版本。
- `dimension`：当前维度 ID 和名称。
- `is_applicable`：当前维度是否适用。

脚本不把结果映射成分数。“不适用”必须保持独立，任何通过率、召回率和误召率统计都不得把“不适用”当成“对”。
